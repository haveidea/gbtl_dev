...
initial begin
  ...
  if(slave_write) begin
      mem[slave_address % CAPACITY]<= #0.1 slave_writedata;
  end

  always @ (posedge clk)
  if(slave_read_active) begin
       slave_readdata_temp <= #0.1 mem[slave_address % CAPACITY ];
  end
...
always @ (posedge clk)
if(reset)
  slave_read_d <= #0.1 1'b0;
else
  slave_read_d <= #0.1 slave_read_active;
...
wire  slave_waitrequest_temp;
  ...
  fifo_fwft #(.DATA_WIDTH(512), .DEPTH_WIDTH(20))
  u_fifo
  (
    .clk(clk),
    .rst(reset),
...
always @(posedge clk) begin
    my_rand1 <= #0.1 $random%2 ;
    my_rand2 <= #0.1 $random%2 ;
end

`ifdef RANDOM_SLV
assign slave_waitrequest = slave_waitrequest_temp | my_rand1;
`else
assign slave_waitrequest = slave_waitrequest_temp;
`endif
